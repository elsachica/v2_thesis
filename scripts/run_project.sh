#!/usr/bin/env bash
# Kör hela pipelinen: rådata -> student_features -> KMeans (Parquet) -> stabilitet.
set -e
...
# Vi sätter default här också, men Python-scripten har nu 180 inbyggt som backup.
...
# 1. Preprocess
echo "==> 1/3 Preprocess (--min-reported-lessons $MIN_LESSONS) -> data/processed/student_features.csv"
python3 src/preprocess.py --min-reported-lessons "$MIN_LESSONS"

# 2. Train KMeans (lean, Parquet-utdata)
echo "==> 2/3 Train KMeans (k=$K) -> data/processed/clustered_students.parquet"
python3 src/train_kmeans.py --k "$K" --min-lessons "$MIN_LESSONS"

# 3. Stability (använder samma filter)
echo "==> 3/3 Stability Test (--k-list ${K}) -> output/plots/"
python3 src/test_kmeans_stability.py --min-lessons "$MIN_LESSONS" --k-list "$K"

echo "-------------------------------------------------------"
echo "Klart! Pipelinen kördes med gränsen $MIN_LESSONS lektioner."
echo "Resultat sparat i: data/processed/clustered_students.parquet"
echo "Grafer finns i: output/plots/"
